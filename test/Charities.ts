...
//@ts-ignore
// Waiting to end ValidateCharity.sol to add the interface
...
describe("CharityRegistry", function () {
	...
	it("should add a new charity for donations", async function () {
		const { validateCharities, charityRegistry, owner } =
	...
	it("should make a donation", async function () {
		const { validateCharities, charityRegistry, owner, validatorAddresses } =
			await deployCharitiesFixture();

		const charities = await validateCharities.getCharities();
		const charityId = charities[1].charityId;
		await validateCharities.vote(charityId, true);

		await expect(
			await charityRegistry.makeDonation(charityId, {
				value: ethers.utils.parseEther("1"),
			})
		)
			.to.emit(charityRegistry, "DonationMade")
